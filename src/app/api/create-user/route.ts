...
import { sendOTPOnMail, sendRegistrationSuccessMail } from "@/services/sendgrid/sendEmail";
import { sendOTPOnPhone, sendRegistrationSuccessMessage } from "@/services/twilio/sendOTPOnPhone";
...
async function verifyPreviousData(email: string, phone: string) {
    ...
    const query = 'SELECT * FROM user_details WHERE (email = ? OR phone = ?) AND is_email_verified = 1 AND is_phone_verified = 1'
...
export async function POST(req: Request) {
    try {
        ...
        const { name, email, phone, address_line, country, state, city, pin_code, profile_photo } = await req.json();
        ...
        const db = await database.getConnection()
        const query = 'INSERT INTO user_details (user_id, full_name, email, phone, profile_photo, email_otp, phone_otp) VALUES (?, ?, ?, ?, ?, ?, ?)'
        const [user] = await db.execute(query, [user_id, name, email.toLowerCase(), phone, profile_photo, EmailOTP, PhoneOTP])
        // console.log(user);
        const address_id = uuidv4()
        const query2 = 'INSERT INTO address_details (address_id ,user_id, address_line, country, state, city, pin_code) VALUES (?, ?, ?, ?, ?, ?, ?)'
        const [address] = await db.execute(query2, [address_id, user_id, address_line, country, state, city, pin_code])
